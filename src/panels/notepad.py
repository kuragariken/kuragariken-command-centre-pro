...
class NoteTab(QWidget):
    ...
    def __init__(self, title="Note", content=""):
        ...
        tb.setStyleSheet("background:qlineargradient(x1:0,y1:0,x2:0,y2:1,stop:0 rgba(14,20,30,0.92),stop:1 rgba(6,10,18,0.85)); border-top:1px solid rgba(255,255,255,0.06); border-bottom:1px solid rgba(255,255,255,0.07);")
        tl = QHBoxLayout(tb)
        ...
        for label, fn in [
            ...
            b.setFixedHeight(26)
            b.setStyleSheet(
                "QPushButton{background:#0d1520;color:#6e7d90;border:1px solid #1a2535;"
...
class NotepadWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        new_btn.setStyleSheet(
            ...
            "border-radius:8px;padding:0 10px;font-size:10px;font-weight:700;}"
        ...
        for col, hov, slot in [
            ...
            b.setStyleSheet(
                f"QPushButton{{background:{col};border:none;border-radius:8px;}}"
    ...
    def _make_search_bar(self) -> QWidget:
        ...
        sty = ("QLineEdit{background:#0a1020;color:#c8d8e8;border:1px solid #1a2535;"
               "border-radius:8px;padding:4px 10px;font-size:11px;font-family:'Segoe UI Variable Text','Segoe UI Variable','Inter','Segoe UI',sans-serif;}"
        ...
        cx.setStyleSheet("QPushButton{background:transparent;color:#6e7d90;border:none;}"
...
class NotepadPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        sub.setStyleSheet("background:transparent;color:#6e7d90;font-size:11px;border:none;")
        il.addWidget(sub)
    ...
    def refresh(self):
        # Called when the Notepad nav pill is clicked — open/raise the window.
        if not self._win:
        ...
        if not self._win.isVisible():
            # Throw out from roughly the top-centre of the app window.
            try:
                center = self.app.mapToGlobal(
                    QPoint(self.app.width() // 2, 60))
            except Exception:
                center = QPoint(600, 300)
            self._win.throw_out(center)
        else:
            self._win.raise_()
