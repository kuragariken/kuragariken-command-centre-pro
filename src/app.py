...
class CommandCentreApp(QMainWindow):
    ...
    def _make_title_bar(self) -> QWidget:
        ...
        self._ka_label.setStyleSheet(
            "background:transparent;color:#3a4e64;"
    ...
    def _toggle_keep_alive(self):
        ...
        if self._ka_worker is None:
            ...
            self._ka_worker.status_update.connect(lambda msg: None)
    ...
    def _nav_to(self, key: str):
        ...
        # Popout panels — open window, return to commands, NO transition
        _popout_keys = {'settings', 'vault', 'notepad', 'pcms'}

        panel = self._panels.get(key)
        if panel:
            idx = self._stack.indexOf(panel)
            if hasattr(self, '_panel_trans') and key not in _popout_keys:
        ...
        for k, btn in self._nav_pill_btns.items():
            if k == key:
                btn.setStyleSheet(
                    ...
                    f"border-radius:12px;padding:2px 14px;font-size:10px;"
                ...
                btn.setStyleSheet(
                    ...
                    f"border-radius:12px;padding:2px 12px;"
