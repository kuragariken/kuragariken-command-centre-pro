...
class CommandPalette(QWidget):
    ...
    def _render_results(self):
        ...
        if not self._results:
            ...
            empty.setStyleSheet(
                "background:transparent;color:#3a4e64;font-size:13px;"
    ...
    def _make_result_row(self, label, text, cat, selected):
        ...
        if selected:
            row.setStyleSheet(
                ...
                f"border:1px solid {accent};border-radius:10px;text-align:left;"
                f"padding:0 16px;}}")
        else:
            row.setStyleSheet(
                "QPushButton{background:transparent;border:1px solid transparent;"
                "border-radius:10px;text-align:left;padding:0 16px;}"
