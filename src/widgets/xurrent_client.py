...
def build_open_count_query(cursors: dict = None, done: set = None):
    """
    Count currently-OPEN requests per tracked team (POS Support, POS EFT).
    Open = any status that isn't completed or declined.

    Paginated the same way as build_analytics_query: teams already fully paged
    (in `done`) are skipped, and `cursors` carries each team's next page
    cursor. This is what makes the count and breakdown exact rather than
    capped at the first 100 — a team can have any number of open requests and
    every one of them gets counted.

    4me's exact RequestStatus enum (confirmed directly from this instance's
    own validation error, since the documented list didn't quite match):
      declined, on_backlog, assigned, accepted, in_progress, waiting_for,
      waiting_for_customer, reservation_pending, workflow_pending,
      project_pending, completed.
    'declined' and 'completed' are excluded as closed; everything else counts
    as open.
    """
    done  = done or set()
    open_statuses = ("on_backlog assigned accepted in_progress waiting_for "
                     "waiting_for_customer reservation_pending "
                     "workflow_pending project_pending")
    open_list = "[" + ", ".join(open_statuses.split()) + "]"
    parts = []
    for i, (name, tid) in enumerate(TEAM_IDS.items()):
        alias = f"o{i}"
        if alias in done:
        ...
        after  = f', after: "{cursor}"' if cursor else ""
        parts.append(f"""
  {alias}: requests(first:100{after}, filter: {{ team: {{ values: ["{tid}"] }},
    status: {{ values: {open_list} }} }}) {{
    totalCount
    nodes {{ requestId status member {{ name }} }}
    pageInfo {{ hasNextPage endCursor }}
  }}""")
    if not parts:
        return None
    return "query OpenCounts {" + "".join(parts) + "\n}"


def team_alias_map():
    """Return {alias: team_name} so callers can label the open-count results."""
    return {f"o{i}": name for i, name in enumerate(TEAM_IDS.keys())}


# ── Store / till classification ──────────────────────────────────────────
# 4me's "Requested for" Person on a POS ticket is registered per-store, with
# a name like "GC81 PnP QualiSave Cosmo Mall" — store code, banner, location,
# all in one plain-text field (confirmed directly from a real ticket, not
# guessed). We read the store code and banner straight out of that text
# instead of maintaining a separate store-code lookup table that could go
# stale or be wrong.
#
# Keyword match is case-insensitive and checked in this order; first hit
# wins. Anything that matches none of these five is bucketed as "Other"
# (covers QualiSave, Boxer, plain PnP supermarkets, etc.) rather than being
# silently mis-classified into one of the five.
STORE_FORMAT_KEYWORDS = [
    ("clothing", "Clothing"),
    ("liquor",   "Liquor"),
    ("express",  "Express"),
    ("local",    "Local"),
...
def classify_store_name(requested_for_name: str):
    """
    Split "GC81 PnP QualiSave Cosmo Mall" into:
      store_code = "GC81"   (first whitespace-delimited token)
      format     = "Other"  (no Hyper/Express/Local/Liquor/Clothing keyword
                              found in the remaining text)
    Returns (store_code, format_label). If the name is empty/unparseable,
    store_code is "" and format is "Unknown" — never silently guessed.
    """
    name = (requested_for_name or "").strip()
    if not name:
        return "", "Unknown"
    store_code = name.split()[0]
    lower = name.lower()
    fmt = "Other"
    for kw, label in STORE_FORMAT_KEYWORDS:
        if kw in lower:
            fmt = label
            break
    return store_code, fmt


def extract_till_number(custom_fields):
    """
    Find the 'Till Number' custom field among a request's customFields list
    and return it as an int, or None if not present / not numeric. We match
    on the field id containing 'till' (case-insensitive) rather than a single
    hardcoded id string, since the exact internal id 4me assigned to this
    field on your instance isn't documented anywhere we can verify — this
    still finds it correctly regardless of the exact slug, and returns None
    (never a guess) if nothing matches.
    """
    for f in (custom_fields or []):
        fid = str(f.get("id") or "")
        if "till" in fid.lower():
            val = f.get("value")
            try:
...
def build_calls_breakdown_query(start_date: str, cursors: dict = None, done: set = None):
    """
    ALL calls (no status filter — open and completed both) created on/after
    start_date, per tracked team, paginated the same way as the other queries
    here. Fetches exactly what's needed to classify store/till: requestId,
    the requester's name (for store code + format), and the request's custom
    fields (for till number).
    """
    done  = done or set()
    parts = []
    for i, (name, tid) in enumerate(TEAM_IDS.items()):
        ...
        parts.append(f"""
  {alias}: requests(first:100{after}, filter: {{ team: {{ values: ["{tid}"] }},
    createdAt: {{ greaterThanOrEqualTo: "{start_date}T00:00:00Z" }} }},
    ...
    if not parts:
        return None
    return "query CallsBreakdown {" + "".join(parts) + "\n}"


def fetch_calls_breakdown(token, account, start_date):
    """
    Fully paginated fetch of every call (any status) since start_date for
    both tracked teams, classified by store code, till parity, and format.

    Returns {team_name: {
        "total": int,
        "calls": [ {request_id, store_code, till_number, parity, format}, ... ],
        "format_counts": {label: n, ...},
        "parity_counts": {"Odd": n, "Even": n, "Unknown": n},
    }}
    Raises on connection/auth error (caller shows it).
    """
    keys = [f"c{i}" for i in range(len(TEAM_IDS))]
    alias_to_name = {f"c{i}": name for i, name in enumerate(TEAM_IDS.keys())}

    all_nodes = {k: [] for k in keys}
    totals    = {}
    cursors   = {}
    done      = set()
    pages     = 0
    MAX_PAGES = 100   # 100 * 100 = 10,000 calls per team per fetch — generous
                       # safety ceiling, never expected to actually engage.

    while pages < MAX_PAGES:
        query = build_calls_breakdown_query(start_date, cursors, done)
        if query is None:
            break
        d = _gql(token, account, query)
        pages += 1
        for alias in keys:
            if alias in done:
                continue
            block = d.get(alias) or {}
            all_nodes[alias].extend(block.get("nodes", []))
            tc = block.get("totalCount")
            if isinstance(tc, int):
                totals[alias] = tc
            pi = block.get("pageInfo", {})
            if pi.get("hasNextPage"):
                cursors[alias] = pi.get("endCursor", "")
            else:
    ...
    for alias, name in alias_to_name.items():
        nodes = all_nodes[alias]
        calls = []
        format_counts = {}
        parity_counts = {"Odd": 0, "Even": 0, "Unknown": 0}
        for n in nodes:
            req_id = n.get("requestId", "")
            requester_name = (n.get("requestedFor") or {}).get("name", "")
            store_code, fmt = classify_store_name(requester_name)
            till = extract_till_number(n.get("customFields"))
            if till is None:
                parity = "Unknown"
            else:
                parity = "Even" if till % 2 == 0 else "Odd"
            calls.append({
                "request_id": req_id,
        ...
        out[name] = {
            "total": totals.get(alias, len(nodes)),
            "calls": calls,
...
def fetch_open_counts(token, account):
    """
    Run the open-count query, following pagination until every team's open
    requests have been fully fetched, and return per team:
        {"total": int, "breakdown": {status: {"member": n, "unclaimed": n}}}

    No cap here — every open request for both teams is counted (4me limits
    a single page to 100, so a team with, say, 340 open requests takes 4
    pages; we keep paging with the same cursor pattern build_analytics_query
    already uses until pageInfo.hasNextPage is false for every team).

    total is the connection's totalCount (always exact). breakdown is built
    from every node actually fetched across all pages, so with full
    pagination it's exact too, not a sample.
    Raises on connection/auth error (caller shows it).
    """
    amap = team_alias_map()
    keys = list(amap.keys())
    ...
    pages      = 0
    MAX_PAGES  = 50   # 50 * 100 = 5,000 open requests per team — a safety
                       # ceiling so a schema quirk can never spin forever;
                       # any real support team's open queue is far below this.

    while pages < MAX_PAGES:
        query = build_open_count_query(cursors, done)
        if query is None:
            break
        d = _gql(token, account, query)
        pages += 1
    ...
    for alias, name in amap.items():
        ...
        for n in nodes:
            ...
            has_member = bool((n.get("member") or {}).get("name"))
            bucket = breakdown.setdefault(st, {"member": 0, "unclaimed": 0})
            bucket["member" if has_member else "unclaimed"] += 1
        out[name] = {"total": total, "breakdown": breakdown}
    return out
